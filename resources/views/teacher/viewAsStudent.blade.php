                                    ...
                                    <div class="col-md-12" >
                                        ...
                                        @foreach($chapters as $chapter)
                                            ...
                                            <form class="QUESTIONS">
                                                <input id="form_chapter" name="form_chapter" type="text" value="{{key($chapter)}}" required hidden/>
                                                <input id="form_keynotes" name="form_keynotes" type="text" value="" required hidden/>
                                                @foreach($chapter[key($chapters[0])]["quiz"] as $question)
                                                            <label class="control-label">{{$question["Q"]}}</label><br>
...
form.addEventListener("submit", function(event) {
  ...
  var  data = new FormData(form);
    ...
    path = document.getElementById("form_chapter").value;

    $.ajax({
        method: 'POST', // Type of response and matches what we said in the route
        url: '/teacher/checkAnswer/' + path, // This is the url we gave in the route
        data: object, // a JSON object to send back
        success: function(response){ // What to do if we succeed
            console.log(response['form_chapter']);
        },
